...
class XmlApiGenerator {
   ...
   * Uses IP-TAG envelope (no <version> element).
   * @param {Array} entries - Tag register entries
   * @returns {string[]} Array of complete uid-message XML strings
   */
  buildTagRegisterMessages(entries) {
    return this._chunkAndBuild(entries, buildTagRegisterPayload, buildTagUidMessage);
  }
  ...
  buildTagUnregisterMessages(entries) {
    return this._chunkAndBuild(entries, buildTagUnregisterPayload, buildTagUidMessage);
  }

  /**
   * Split entries into chunks and build uid-message XML for each chunk.
   * @param {Array} entries - Entries to chunk
   * @param {Function} buildFn - Payload builder function
   * @param {Function} [envelopeFn=buildUidMessage] - Envelope wrapper function
   * @private
   */
  _chunkAndBuild(entries, buildFn, envelopeFn) {
    const wrapFn = envelopeFn || buildUidMessage;
    const messages = [];
    for (let i = 0; i < entries.length; i += this._chunkSize) {
      const chunk = entries.slice(i, i + this._chunkSize);
      messages.push(wrapFn(buildFn(chunk)));
    }
    return messages;
  }
}
