...
/**
 * Build a <register>...</register> payload for IP-TAG operations.
 *
 * PAN-OS IP-TAG format places <register> directly inside <payload>,
 * with no <tag> wrapper and no <version> element in the envelope.
 *
 ...
 *   // => '<register><entry ip="10.1.1.1"><tag><member>web-server</member><member>production</member></tag></entry></register>'
 */
function buildTagRegisterPayload(entries) {
  ...
  return `<register>${items.join('')}</register>`;
}

/**
 * Build an <unregister>...</unregister> payload for IP-TAG operations.
 *
 * PAN-OS IP-TAG format places <unregister> directly inside <payload>,
 * with no <tag> wrapper and no <version> element in the envelope.
 *
 * @param {Array<{ip: string, tags: string[]}>} entries
 * @returns {string} XML fragment
...
function buildTagUnregisterPayload(entries) {
  ...
  return `<unregister>${items.join('')}</unregister>`;
}
 ...
 * Per PAN-OS API documentation, the envelope contains only <type> and <payload>
 * (no <version> element).
 *
 * @param {string} payloadXml - Inner payload XML (login, logout, groups, register, unregister)
 * @returns {string} Complete uid-message XML
 *
 * @example
 *   buildUidMessage(buildLoginPayload([...]))
 *   // => '<uid-message><type>update</type><payload>...</payload></uid-message>'
 */
...
/**
 * Alias for buildUidMessage — kept for backward compatibility.
 * All PAN-OS uid-message envelopes use the same format (no <version>).
 *
 * @param {string} payloadXml - Inner payload XML
 * @returns {string} Complete uid-message XML
 */
const buildTagUidMessage = buildUidMessage;
