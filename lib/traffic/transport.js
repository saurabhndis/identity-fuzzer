...
class TCPTransport {
  ...
  connect(host, port, opts = {}) {
    return new Promise((resolve, reject) => {
      ...
      // Set DSCP via iptables BEFORE connect (so SYN packet has DSCP)
      let iptablesRule = null;
      if (this._dscp && process.platform === 'linux') {
        try {
          const { execSync } = require('child_process');
          const dscpClass = this._dscp >> 2; // TOS byte to DSCP value (upper 6 bits)
          iptablesRule = `-p tcp -d ${host} --dport ${port} -j DSCP --set-dscp ${dscpClass}`;
          execSync(`sudo -n iptables -t mangle -A OUTPUT ${iptablesRule} 2>/dev/null`, { timeout: 2000 });
          this._appliedTOS = this._dscp;
          this._tosMethod = 'iptables mangle';
        } catch (_) {
          iptablesRule = null;
          // Fall through to post-connect setsockopt
        }
      }
      ...
      const timer = setTimeout(() => {
        ...
        if (iptablesRule) {
          try { require('child_process').execSync(`sudo -n iptables -t mangle -D OUTPUT ${iptablesRule} 2>/dev/null`, { timeout: 2000 }); } catch (_) {}
        }
      ...
      this._socket.on('connect', () => {
        clearTimeout(timer);
        // Remove iptables rule (no longer needed, setsockopt will maintain DSCP)
        if (iptablesRule) {
          try { require('child_process').execSync(`sudo -n iptables -t mangle -D OUTPUT ${iptablesRule} 2>/dev/null`, { timeout: 2000 }); } catch (_) {}
        }
        // Apply DSCP via setsockopt for all subsequent packets
        if (this._dscp) {
          const tosResult = trySetTOS(this._socket, this._dscp);
          if (tosResult.success) {
            this._appliedTOS = tosResult.readback || this._dscp;
            this._tosMethod = iptablesRule ? 'iptables + native setsockopt' : tosResult.method;
          } else if (!iptablesRule) {
  ...
  connect(host, port, opts = {}) {
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        reject(new Error(`TLS connect timeout to ${host}:${port} after ${timeout}ms`));
      }, timeout);

      // Step 1: Set DSCP via iptables BEFORE connect (so SYN has DSCP)
      this._appliedTOS = 0;
      this._tosError = null;
      this._tosMethod = 'none';
      let iptablesRule = null;
      ...
      const tcpSocket = net.createConnection({ host, port, allowHalfOpen: opts.allowHalfOpen !== false }, () => {
        // Step 2: Remove iptables rule and set DSCP via setsockopt for ongoing packets
        if (iptablesRule) {
          try { require('child_process').execSync(`sudo -n iptables -t mangle -D OUTPUT ${iptablesRule} 2>/dev/null`, { timeout: 2000 }); } catch (_) {}
        }
        if (this._dscp) {
          const tosResult = trySetTOS(tcpSocket, this._dscp);
          if (tosResult.success) {
            this._appliedTOS = tosResult.readback || this._dscp;
            this._tosMethod = iptablesRule ? 'iptables + native setsockopt' : tosResult.method;
          }
        ...
        // Step 3: Upgrade to TLS on the existing TCP socket
        const tlsOpts = {
          socket: tcpSocket,  // Use the existing TCP socket with DSCP already set
          host,               // SNI hostname
        ...
        if (opts.certFile) {
          try { tlsOpts.cert = fs.readFileSync(opts.certFile); } catch (e) {
            if (!settled) { settled = true; clearTimeout(timer); reject(new Error(`Failed to read TLS cert file: ${e.message}`)); }
            return;
          }
        }
        if (opts.keyFile) {
          try { tlsOpts.key = fs.readFileSync(opts.keyFile); } catch (e) {
            if (!settled) { settled = true; clearTimeout(timer); reject(new Error(`Failed to read TLS key file: ${e.message}`)); }
            return;
          }
        }
        if (opts.caFile) {
          try { tlsOpts.ca = fs.readFileSync(opts.caFile); } catch (e) {
            if (!settled) { settled = true; clearTimeout(timer); reject(new Error(`Failed to read TLS CA file: ${e.message}`)); }
            return;
          }
        }
        if (opts.alpn) {
        ...
        try {
          ...
          this._socket.on('error', (err) => {
            ...
            reject(new Error(`TLS handshake failed to ${host}:${port}: ${err.message}`));
          });
        } catch (err) {
          if (!settled) {
            settled = true;
            clearTimeout(timer);
            this._state = TRANSPORT_STATE.ERROR;
            reject(new Error(`TLS connect error to ${host}:${port}: ${err.message}`));
          }
        }
      });
