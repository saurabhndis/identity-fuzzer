...
class TrafficFuzzerClient {
  ...
  /**
   * Abort the currently running scenario and any future ones.
   */
  abort() {
    this._aborted = true;
    // Force-close any active transport connections
    if (this._activeTransport) {
      try { this._activeTransport.close(); } catch (_) {}
      this._activeTransport = null;
    }
    for (const t of this._activeTransports) {
      try { t.close(); } catch (_) {}
    }
    this._activeTransports = [];
    if (this.pcap) {
    ...
    try {
      for (const step of scenario.steps) {
        ...
        if (this._aborted) {
          ...
          result.response = 'Aborted by user';
          this.logger.info(`  ⚠ Scenario aborted by user`);
          break;
        }
        ...
        switch (step.type) {
          case 'connect': {
            // Apply TLS override from GUI toggle:
            // useTls=true  → force TLS for all connections (plain→tls)
            // useTls=false → force plain TCP for all connections (tls→plain, h2→h2c)
            // useTls=undefined → use scenario's default mode
            const scenarioMode = step.mode || 'plain';
            const mode = (this.useTls === true) ? 'tls' : (this.useTls === false) ? 'plain' : scenarioMode;
            ...
            const tlsLabel = mode === 'tls' ? ' 🔒' : '';

            transport = createTransport(mode, { dscp });
            this._activeTransport = transport;
            this.logger.info(`${stepPrefix} CONNECT → ${this.host}:${port} (${mode.toUpperCase()}${alpnLabel}, DSCP=${dscpLabel})${tlsLabel}`);
          ...
          case 'parallel-connect': {
            ...
            const pScenarioMode = step.mode || 'plain';
            const mode = (this.useTls === true) ? 'tls' : (this.useTls === false) ? 'plain' : pScenarioMode;
            const port = step.port || this.port;
      ...
      // Only set PASSED if not already set to ABORTED
